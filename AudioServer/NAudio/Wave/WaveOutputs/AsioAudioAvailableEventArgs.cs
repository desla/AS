...
namespace NAudio.Wave
{
    ...
    public class AsioAudioAvailableEventArgs : EventArgs
    {
        ...
        /// <summary>
        /// Для работы с AsioOutExtension.
        /// </summary>
        /// <param name="samples"></param>
        /// <returns></returns>
        public int GetAsInterleavedSamplesExtended(float[] samples, int aChannel)
        {
            var currentInputChannels = new IntPtr[] {InputBuffers[aChannel]};
            int channels = currentInputChannels.Length;
            ...
            unsafe {
                if (AsioSampleType == AsioSampleType.Int32LSB) {
                    for (int n = 0; n < SamplesPerBuffer; n++) {
                        for (int ch = 0; ch < channels; ch++) {
                            samples[index++] = *((int*)currentInputChannels[ch] + n) / (float)Int32.MaxValue;
                        }
                    }
                }
                else if (AsioSampleType == AsioSampleType.Int16LSB) {
                    for (int n = 0; n < SamplesPerBuffer; n++) {
                        for (int ch = 0; ch < channels; ch++) {
                            samples[index++] = *((short*)currentInputChannels[ch] + n) / (float)Int16.MaxValue;
                        }
                    }
                }
                else if (AsioSampleType == AsioSampleType.Int24LSB) {
                    for (int n = 0; n < SamplesPerBuffer; n++) {
                        for (int ch = 0; ch < channels; ch++) {
                            byte* pSample = ((byte*)currentInputChannels[ch] + n * 3);
                    ...
                    for (int n = 0; n < SamplesPerBuffer; n++) {
                        for (int ch = 0; ch < channels; ch++) {
                            samples[index++] = *((float*)currentInputChannels[ch] + n);
                        }
                    }
                }
                else {
