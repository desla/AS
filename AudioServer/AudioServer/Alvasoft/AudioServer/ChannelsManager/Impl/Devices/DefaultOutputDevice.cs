...
{
    /// <summary>
    /// Физическое устройство - двухканальная звуковая карта.
    /// </summary>
    public class DefaultOutputDevice : IOutputDevice
    {
        private int samplePerSec = 44100; // Сэмплов в секунду
        ...
        public static DefaultOutputDevice FindPhysicalDeviceByName(string aDeviceName)
        {
            int channelCount;
            int deviceId;
            ...
            return new DefaultOutputDevice(aDeviceName, deviceId, channelCount);
        }
        ...
        {
            if (aChannelId < 0 || aChannelId >= channelsCount) {
                throw new ArgumentException("channelId");
            }

            return channelLevels[aChannelId];
        }
        ...
        private DefaultOutputDevice(string aDeviceName, int aDeviceId, int aChannelCount)
        {
